...
//----- other globals------------------------------
static uint8_t raceMode = 0; // 0: race mode is off; 1: lap times are counted relative to last lap end; 2: lap times are relative to the race start
                             // (sum of all previous lap times);
// static uint8_t isSoundEnabled = 1; // TODO: implement this option
static uint8_t isConfigured =
    0; // changes to 1 if any input changes the state of the device. it will mean that externally stored preferences should not be applied
static uint8_t shouldWaitForFirstLap =
    0; // 0 means start table is before the laptimer, so first lap is not a full-fledged lap (i.e. don't respect min-lap-time for the very first lap)
...
#define RISE_RSSI_THRESHOLD_PERCENT                                                                                                                  \
  25 // rssi value should pass this percentage above low value to continue finding the peak and further fall down of rssi
...
void handleSerialControlInput(char *controlData, uint8_t ControlByte, uint8_t NodeAddr, uint8_t length) {
  ...
  if (length > 4) { // set value commands  changed to n+1 ie, 3+1 = 4.
    ...
    case CONTROL_THRESHOLD_SETUP: // setup threshold using sophisticated algorithm
      ...
      if (!raceMode) { // don't run threshold setup in race mode because we don't calculate slowRssi in race mode, but it's needed for setup threshold
                       // algorithm
...
bool isInRaceMode() { return raceMode; }
